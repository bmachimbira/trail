...
export class FileStorage extends Context.Tag("trail/FileStorage")<FileStorage, FileStorageService>() {}
...
export const FileStorageLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const put = (id: string, content: Uint8Array) =>
      fs.writeFile(blobPath(id), content).pipe(
        Effect.mapError((cause) => new StorageError({ op: "write", cause })),
      );

    const get = (id: string): Effect.Effect<Uint8Array, StorageError | NotFoundError> =>
      safeId(id)
        ? fs.readFile(blobPath(id)).pipe(
    ...
    const remove = (id: string) =>
      fs.remove(blobPath(id), { force: true }).pipe(
        Effect.mapError((cause) => new StorageError({ op: "remove", cause })),
      );
